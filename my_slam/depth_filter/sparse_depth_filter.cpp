...
namespace my_slam
{
    ...
    // Seed::Seed(float depth_mean, float depth_min) :
    ...
    // {}
    // /**************** depth filter ********************/
    // sparse_depth_filter::sparse_depth_filter(camera *cam):
    // cam_(cam)
    // {
    //     init();
    // }
    // void sparse_depth_filter::init()
    // {
    ...
    // }
    // void sparse_depth_filter::addFrame(cv::Mat frame)
    // {


    // }
    // void sparse_depth_filter::addKeyframe(cv::Mat frame, double depth_mean, double depth_min)
    // {

    // }

    // void sparse_depth_filter::update_seeds(cv::Mat frame)
    // {
    //     boost::unique_lock<boost::mutex> lock(seeds_mut_);
    //     std::list<Seed>::iterator it=seeds_.begin();
    //     // 当前相机焦距
    //     const double focal_length = cam_->get_focal_length();
    //     double px_noise = 1.0;
    //     // 设置当前误差为一个像素点引起的角度误差
